        ...
        if not(preset == False):
            for item in setup.namelist():
                if item.startswith(preset) == True and ('config.txt' in item) == False:
                    ...
                    if os.path.exists(pathkodi + os.sep + item2) or os.path.isfile(pathkodi + os.sep + item2):
                        ...
                        if ('addons' in root) == True and ('repository' in root) == True:
                            for repo in repos:
                                if (repo in root) == True:
                                    ...
                                    checkup.write(temp, os.path.relpath(temp, pathkodi))
                                    c+=1
                        if ('userdata' in root) == True and (str(now) in str(file)) == False and (str(now) in str(root)) == False:
                            temp = os.path.join(root, file)
                            checkup.write(temp, os.path.relpath(temp, pathkodi))
                            c+=1
