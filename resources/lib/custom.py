...
def CustomizeAddons(finalList, lastusedaddonconfig, pathkodi, setupchoice):
    ...
    if (setupchoice == 1):
        presetchoice = dialog.ChoosePreset(skinpicked)
        ...
        # for newaddon in newaddons:
        #     optlist.append(sort.configitemreplace(newaddon))
        # newaddons = optlist
        
        # for newaddon in newaddons:
        #     i = newaddons.index(newaddon)
        #     newaddon = sort.reverseconfigitemreplace(newaddon)
        #     newaddons[i] = newaddon
    ...
    s = '[COLOR firebrick]CURRENT CONFIG[/COLOR]\n'
    for i in config:
        s += '[COLOR firebrick]' + str(i) + '[/COLOR]' + '\n'
    ...
    return config, presetchoice, repospicked, mainskin