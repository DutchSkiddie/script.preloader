...
def AddonList(zip, lastusedaddons, lastusedaddonconfigtxt, setupchoice, pathkodi, pathpresets):
    ...
    if (setupchoice == 0) or (setupchoice == 1) or (setupchoice == 3):
        configlistnames, presetchoice, repos, mainskin, mainvideoselect = (custom.CustomizeAddons(finalList, lastusedaddonconfig, setupchoice, pathpresets))         
    if (setupchoice == 2):
        mainvideoselect = ''
        presetchoice = []
        temp = []
    ...
    for configlistname in configlistnames:
        if (configlistname == 'plugin.video.fen') or (configlistname == 'plugin.video.fenlight'):
            repos.append(configlistname)
    
    return newaddons, configlistnames, presetchoice, repos, mainvideoselect
